...
def evaluate_model(xgb_params, X_train, y_train, X_val, y_val):
    ...
    y_val_prob = lr_model.predict_proba(X_val_reduced)[:, 1]
    auc_score = roc_auc_score(y_val, y_val_prob)
    return auc_score

def linear_search_tuning(X_train, y_train, X_val, y_val, param_values):
    best_auc = 0
    best_params = None
    ...
    for combination in all_combinations:
        ...
        auc_score = evaluate_model(xgb_params, X_train, y_train, X_val, y_val)
...
def main(df, target_column='target', param_values=None):
    ...
    best_params, best_auc = linear_search_tuning(X_train, y_train, X_val, y_val, param_values)
